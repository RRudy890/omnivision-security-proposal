...
const ProcessSlide: React.FC<SlideProps> = () => {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12 relative">
      <nav className="absolute top-0 w-full px-12 py-8 flex justify-end">
        <img
          src="/logo.png"
          alt="OmniVision Logo"
      ...
      <div className="mt-20 w-full max-w-6xl overflow-hidden border border-midnight-yellow/20 relative h-[300px] rounded">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="bg-midnight-black/80 backdrop-blur-sm border-l-4 border-midnight-yellow p-8 max-w-xl">
            <p className="text-midnight-yellow font-black tracking-[0.3em] text-xs mb-3 uppercase">Elite Response Unit</p>
            <p className="text-3xl font-bold text-white uppercase tracking-tight leading-tight">Zero-Latency Strategic Monitoring Operations</p>
          </div>
        </div>
      </div>
    </div>
...
const ProcessStep = ({ step, icon, title, desc, hasArrow }: any) => (
  <div className="group relative flex flex-col items-center">
    <div className="mb-8 relative">
      <div className="w-24 h-24 bg-midnight-grey border-2 border-midnight-yellow flex items-center justify-center shadow-[0_0_30px_rgba(255,255,0,0.1)] group-hover:bg-midnight-yellow group-hover:text-black transition-all duration-300">
        <span className="material-symbols-outlined text-5xl text-midnight-yellow group-hover:text-black">{icon}</span>
      </div>
